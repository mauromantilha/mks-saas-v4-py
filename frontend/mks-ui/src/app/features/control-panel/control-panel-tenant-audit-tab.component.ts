...
@Component({
  selector: "app-control-panel-tenant-audit-tab",
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: "./control-panel-tenant-audit-tab.component.html",
  styleUrl: "./control-panel-tenant-audit-tab.component.scss",
})
export class ControlPanelTenantAuditTabComponent implements OnChanges {
  private readonly destroyRef = inject(DestroyRef);

  @Input({ required: true }) tenantId!: number;

  readonly displayedColumns = ["created_at", "actor", "action", "entity", "correlation_id"];
  ...
  private loadEvents(resetPage = false): void {
    ...
    this.auditApi
      ...
      .subscribe({
        ...
        error: () => {
          this.error.set("Falha ao carregar auditoria do tenant.");
          this.events.set([]);
